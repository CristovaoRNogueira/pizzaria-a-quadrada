...
import { Trash2, Plus, Minus, ArrowLeft, ShoppingBag, MapPin, Store, CreditCard, Banknote, QrCode } from 'lucide-react';
...
const Cart: React.FC = () => {
  ...
  const handleSubmitOrder = (e: React.FormEvent) => {
    ...
    if (customer.deliveryType === 'delivery' && (!customer.address || !customer.neighborhood)) {
      dispatch({
        type: 'ADD_NOTIFICATION',
    ...
    dispatch({ type: 'ADD_NOTIFICATION', payload: 'Pedido enviado com sucesso!' });
    dispatch({ type: 'SET_VIEW', payload: 'menu' });
    
    // Simulate WhatsApp notification
    setTimeout(() => {
      dispatch({ type: 'ADD_NOTIFICATION', payload: '📱 WhatsApp: Seu pedido foi recebido!' });
    }, 1000);
  };

  const handlePaymentSuccess = (paymentData: PaymentInfo) => {
    ...
    dispatch({ type: 'CREATE_ORDER', payload: order });
    dispatch({ type: 'ADD_NOTIFICATION', payload: 'Pedido e pagamento processados com sucesso!' });
    dispatch({ type: 'SET_VIEW', payload: 'menu' });
  };
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="text-center">
          <div className="mb-6">
            ...
            <p className="text-gray-600 mb-6">Adicione algumas pizzas deliciosas ao seu carrinho!</p>
          </div>
          <button
          ...
          <form onSubmit={handleSubmitOrder} className="space-y-4">
            ...
            {customer.deliveryType === 'pickup' && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <h4 className="font-semibold text-blue-800 mb-2">📍 Endereço para Retirada:</h4>
                ...
                <div className="flex items-center space-x-3">
                  <input
                    ...
                    onChange={(e) => setPayment({ 
                      ...
                      changeAmount: e.target.checked ? payment.changeAmount : undefined
                    })}
                    className="rounded border-gray-300 text-red-600 focus:ring-red-500"
                  />
                  <label htmlFor="needsChange" className="text-sm font-medium text-gray-700">
                    Preciso de troco
                  </label>
                </div>
            ...
            <div className="border-t pt-4">
              ...
              {payment.method === 'cartao' ? (
                ...
                >
                  ...
                  <span>Confirmar Pedido - {getPaymentMethodLabel(payment.method)}</span>
                </button>
              )}
            </div>
          </form>
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="space-y-4">
        {state.cart.map((item) => (
          <div key={`${item.id}-${item.selectedSize}`} className="bg-white rounded-xl shadow-md p-4">
      ...
      <div className="mt-8 bg-white rounded-xl shadow-lg p-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <span className="text-3xl font-bold text-red-600">R$ {total.toFixed(2)}</span>
        </div>
